...
describe('recipes-model', () => {
  ...
  describe('getFirstOrderRelatives(id, maxLength)', () => {
    it('should return an array of first-order relatives', async () => {
      const expectedFirstOrderRelativesOfRecipeTwo = [
        {id: 1, name: 'sweet crepes', parent_id: 1},
      ...
      const actualFirstOrderRelativesOfRecipeTwo = await Recipes.getFirstOrderRelatives(2);

      expect(actualFirstOrderRelativesOfRecipeTwo).toEqual(expectedFirstOrderRelativesOfRecipeTwo);
    });

    it('should return an array with the specified max_length', async () => {
      const expectedFirstTwoFirstOrderRelativesOfRecipeTwo = [
        {id: 1, name: 'sweet crepes', parent_id: 1},
        {id: 3, name: 'blueberry pancakes', parent_id: 2},
      ];

      const actualFirstTwoFirstOrderRelativesOfRecipeTwo = await Recipes.getFirstOrderRelatives(2, 2);

      expect(actualFirstTwoFirstOrderRelativesOfRecipeTwo).toEqual(expectedFirstTwoFirstOrderRelativesOfRecipeTwo);
    });

    it('should exclude parents if the given recipe is a root', async () => {
      const expectedFirstOrderRelativesOfRecipeOne = [
        {id: 2, name: 'pancakes', parent_id: 1},
        {id: 4, name: 'mini pancakes', parent_id: 1},
        {id: 9, name: 'savoury crepes', parent_id: 1},
      ];

      const actualFirstOrderRelativesOfRecipeOne = await Recipes.getFirstOrderRelatives(1);

      expect(actualFirstOrderRelativesOfRecipeOne).toEqual(expectedFirstOrderRelativesOfRecipeOne);
    });
  });
