...
async function getRecipes() {
  const recipes = await db('recipes as r')
    .leftJoin('recipe_images as rim', 'rim.recipe_id', 'r.id')
    .leftJoin('images as i', 'i.id', 'rim.image_id')
    .select(
      'r.id',
...
async function getRecipeById(id) {
  const recipe = await db('recipes as r')
    .leftJoin('recipe_images as rim', 'rim.recipe_id', 'r.id')
    .leftJoin('images as i', 'i.id', 'rim.image_id')
    .leftJoin('recipe_categories as rc', 'rc.recipe_id', 'r.id')
    .leftJoin('categories as c', 'c.id', 'rc.category_id')
    .select(
      'r.id',
      'r.title as recipe_title',
      'r.description',
  ...
  const tags = await db('recipe_tags as rt')
    .join('recipes as r', 'r.id', 'rt.recipe_id')
  ...
  return { recipe, ingredients, tags, instructions };
}
