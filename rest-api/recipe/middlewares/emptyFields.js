...
function emptyFields(req, res, next) {
  if (!Object.entries(req.body.recipes).length) {
    ...
    req.body.prototype.hasOwnProperty('tags') &&
    !req.body.tags.length
  ) {
    res.status(400).json({ error: 'Tags is empty' });
  } else if (
    req.body.prototype.hasOwnProperty('recipe_tags') &&
    !req.body.recipe_tags.length
  ) {
    res.status(400).json({ error: 'Recipe_tags is empty' });
  } else if (!req.body.images.length) {
    res.status(400).json({ error: 'Images is empty' });
  } else if (
    req.body.prototype.hasOwnProperty('ingredients') &&
    !req.body.ingredients.length
  ) {
    res.status(400).json({ error: 'Ingredients is empty' });
  } else if (
    req.body.prototype.hasOwnProperty('recipe_ingredients') &&
    !req.body.recipe_ingredients.length
  ) {
